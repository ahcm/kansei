//! Command-line frontend shared by the `kansei` and `ks` binaries.
use crate::{ast, eval, intern, lexer, lsp, parser, pm, sexpr, formatter, source_files, test_runner, value, wasm_pm};
...
/// Run the process-oriented CLI using the current process arguments.
///
/// Commands may terminate the process. Rust hosts should use [`crate::Interpreter`]
...
pub fn run() -> rustyline::Result<()>
{
    ...
    interpreter.set_program(crate::Program {
        name: args[0].clone(),
        args: script_args,
        env: env::vars().collect(),
    });
    if let Some(value::Value::Map(program)) = interpreter.get_global("program")
    {
        program.borrow_mut().data.insert(
            intern::intern("exit"),
