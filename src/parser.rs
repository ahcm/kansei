...
impl Parser
{
        ...
        Ok({
            ...
            if self.current_token.token == Token::Equals
            {
                ...
                return Ok(match expr.kind
                {
                    ...
                    {
                        return Err(
                            self.error(format!("Invalid assignment target at line {}", span.line))
                        );
                    }
                });
            }
    ...
    fn parse_math(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            let mut left = self.parse_term()?;

            while self.current_token.token == Token::Plus
                || self.current_token.token == Token::Minus
            {
                let span = self.current_token.clone();
                let op = match self.current_token.token
                {
    ...
    fn parse_term(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            ...
            while self.current_token.token == Token::Star
                || self.current_token.token == Token::Slash
            {
                let span = self.current_token.clone();
                let op = match self.current_token.token
                {
    ...
    fn parse_struct_type_fields(&mut self) -> Result<Vec<(SymbolId, TypeRef)>, ParseError>
    {
        Ok({
    ...
    fn parse_struct_literal_fields(&mut self) -> Result<Vec<(SymbolId, Expr)>, ParseError>
    {
        Ok({
            let mut fields = Vec::new();
            if self.current_token.token != Token::RightBrace
    ...
    fn parse_type_ref(&mut self) -> Result<TypeRef, ParseError>
    {
        Ok({
            ...
            match &self.current_token.token
            {
                ...
                {
                    return Err(self
                        .error(format!("Expected type name at line {}", self.current_token.line)));
                }
            }
            while self.current_token.token == Token::ColonColon
            {
                ...
                match &self.current_token.token
                {
                    ...
                    {
                        return Err(self.error(format!(
                            "Expected type name after :: at line {}",
                            self.current_token.line
                        )));
                    }
                }
    ...
    fn parse_use(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            ...
            match &self.current_token.token
            {
                ...
                {
                    return Err(self.error(format!(
                        "Expected module name after use at line {}",
                        self.current_token.line
                    )));
                }
            }
            ...
            while self.current_token.token == Token::ColonColon
            {
                ...
                match &self.current_token.token
                {
                    ...
                    {
                        return Err(self.error(format!(
                            "Expected module name after :: at line {}",
                            self.current_token.line
                        )));
                    }
                }
    ...
    fn parse_scoped_public(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            let span = self.current_token.clone();
            self.eat()?; // eat '@'
    ...
    fn parse_load(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            ...
            match &self.current_token.token
            {
                ...
                {
                    return Err(self.error(format!(
                        "Expected module name after load at line {}",
                        self.current_token.line
                    )));
                }
            }
    ...
    fn parse_export(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            ...
            match &self.current_token.token
            {
                ...
                {
                    return Err(self.error(format!(
                        "Expected namespace after export at line {}",
                        self.current_token.line
                    )));
                }
            }
            ...
            loop
            {
                ...
                {
                    return Err(self.error(format!(
                        "Expected :: after export namespace at line {}",
                        self.current_token.line
                    )));
                }
                self.eat()?; // ::
                ...
                match &self.current_token.token
                {
                    ...
                    {
                        return Err(self.error(format!(
                            "Expected namespace segment at line {}",
                            self.current_token.line
                        )));
                    }
                }
    ...
    fn parse_if(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            ...
            let else_branch = match self.current_token.token
            {
                ...
                {
                    return Err(self.error(format!(
                        "Expected elif, else, or end at line {}",
                        self.current_token.line
                    )));
                }
            };
    ...
    fn parse_for(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            ...
            let var_name = match &self.current_token.token
            {
                ...
                {
                    return Err(self.error(format!(
                        "Expected identifier after 'for' at line {}",
                        self.current_token.line
                    )));
                }
            };
        ...
        Ok({
            ...
            let var = if self.current_token.token == Token::Pipe
            {
                ...
                let name = match &self.current_token.token
                {
                    ...
                    {
                        return Err(self.error(format!(
                            "Expected loop variable name at line {}",
                            self.current_token.line
                        )));
                    }
                };
    ...
    fn parse_collect(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            let span = self.current_token.clone();
            ...
            let var = if self.current_token.token == Token::Pipe
            {
                ...
                let name = match &self.current_token.token
                {
                    ...
                    {
                        return Err(self.error(format!(
                            "Expected collect variable name at line {}",
                            self.current_token.line
                        )));
                    }
                };
    ...
    fn parse_result(&mut self) -> Result<Expr, ParseError>
    {
        Ok({
            let span = self.current_token.clone();
            self.eat()?; // eat 'result'
            ...
            {
                return Err(
                    self.error(format!("result requires else at line {}", self.current_token.line))
                );
            }
            self.eat()?; // eat 'else'
            ...
            let (else_binding, else_expr) = if self.current_token.token == Token::Pipe
            {
                ...
                let name = match &self.current_token.token
                {
                    ...
                    {
                        return Err(self.error(format!(
                            "Expected error binding name at line {}",
                            self.current_token.line
                        )));
                    }
                };
                ...
                {
                    return Err(self.error(format!(
                        "Expected '{{' after else binding at line {}",
                        self.current_token.line
                    )));
                }
                self.eat()?; // {
    ...
    fn parse_pipe_params(&mut self) -> Result<Vec<Param>, ParseError>
    {
        Ok({
            let mut params = Vec::new();
        ...
        Ok({
            ...
            while i < chars.len()
            {
                ...
                if ch == '{'
                {
                    ...
                    {
                        return Err(self
                            .error(format!("Unclosed format string expression at line {}", line)));
                    }
                    ...
                    {
                        return Err(
                            self.error(format!("Empty format string expression at line {}", line))
                        );
                    }
                ...
                else if ch == '}'
                {
                    ...
                    {
                        return Err(
                            self.error(format!("Unmatched '}}' in format string at line {}", line))
                        );
                    }
                }
                else
    ...
    fn split_format_expr(
        &self,
        input: &str,
        line: usize,
    ) -> Result<(String, Option<FormatSpec>), ParseError>
    {
        Ok({
            ...
            if let Some(idx) = split_at
            {
                ...
                {
                    return Err(self.error(format!("Empty format specifier at line {}", line)));
                }
    ...
    fn parse_format_spec(&self, spec: &str, line: usize) -> Result<FormatSpec, ParseError>
    {
        if let Some(rest) = spec.strip_prefix('.')
        {
            if rest.is_empty() || !rest.chars().all(|c| c.is_ascii_digit())
            {
                return Err(self.error(format!("Invalid format precision at line {}", line)));
            }
            let precision = rest
                .parse::<usize>()
                .map_err(|_| self.error(format!("Invalid format precision at line {}", line)))?;
            return Ok(FormatSpec {
                precision: Some(precision),
            });
        }
        return Err(self.error(format!("Unsupported format specifier at line {}", line)));
    }
}
...
mod tests
{
    ...
    #[test]
    fn recovers_for_next_document_and_reports_location()
    {
        let error = parse_source("puts 1\n  $").unwrap_err();
        assert_eq!((error.line, error.column), (2, 3));
        assert!(parse_source("fn add(x, y) x + y end\nputs add(1, 2)").is_ok());
        assert!(parse_source("if true puts 1 else puts 2 end").is_ok());
    }
}
