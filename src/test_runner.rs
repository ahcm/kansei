    ...
    while let Some(arg) = args.next()
    {
        match arg.as_str()
        {
            "--bytecode" => match args.next().map(String::as_str)
            {
                ...
                _ =>
                {
                    eprintln!("test: --bytecode expects off, simple, advanced, or all");
                    return 2;
                }
            },
            "--timeout" => match args.next().and_then(|v| v.parse::<u64>().ok())
            {
                ...
                _ =>
                {
                    eprintln!("test: --timeout expects positive seconds");
                    return 2;
                }
            },
            "--" =>
            ...
            {
                eprintln!("test: unknown option {value}");
                return 2;
            }
    ...
    for file in files
    {
        for mode in &modes
        {
            ...
            match run_one(&exe, &file, mode, timeout)
            {
                ...
                {
                    failures += 1;
                    eprintln!("FAIL [{}] {}: {error}", mode, file.display());
                }
            }
        }
    }
