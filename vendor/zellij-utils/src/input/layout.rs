...
#[cfg(all(not(target_family = "wasm"), feature = "http-client"))]
...
impl Layout {
    ...
    #[cfg(all(not(target_family = "wasm"), not(feature = "http-client")))]
    pub fn stringified_from_url(_url: &str) -> Result<String, ConfigError> {
        Err(ConfigError::DownloadError(
            "HTTP downloads are disabled (feature 'http-client' not enabled)".to_string(),
        ))
    }
